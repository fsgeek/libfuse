...
//
static int niccolum_find_file_in_path(char **files, char **paths, char *prefix, char **file_found, char **path_found) 
{
...
int niccolum_process_search_request(niccolum_path_search_request_t *search_request, niccolum_path_search_response_t **search_response)
{
    unsigned file_count, path_count;
    unsigned index, index2;
    niccolum_path_search_response_t *rsp = NULL;
    char **file_list = NULL, **path_list = NULL;

    *search_response = NULL;

    if (0 != (search_request->SearchDataFlags & NICCOLUM_SEARCH_DATA_SHARED_MEM)) {
    ...
    file_count = 0;
    for (index = 0; index < search_request->SearchDataLength; index ++) {
        if ('\0' == search_request->SearchData[index]) {
            file_count++;

            if ('\0' == search_request->SearchData[index+1]) {
                // double NULL means end of list
                // TODO: probably should deal with index+1 >= length
                assert(index+1 < search_request->SearchDataLength);
                break;
            }
        }
    }
    ...
    path_count = 0;
    for (; index < search_request->SearchDataLength; index++) {
        if ('\0' == search_request->SearchData[index]) {
            path_count++;
    ...
    // validate
    // TODO: do we want a max size?  Any other validation?
    if ((0 == file_count) || (0 == path_count)) {
        errno = EINVAL;
        return -1;
    }

    file_list = (char **)malloc(sizeof(char *) * (1 + file_count));
    path_list = (char **)malloc(sizeof(char *) * (1 + path_count));

    while ((NULL != file_list) && (NULL != path_list)) {

        index2 = 0;
        index = 0;
        
        do {
            file_list[index2++] = &search_request->SearchData[index];

            while ('\0' != search_request->SearchData[index]) {
                index++;
                assert(index < search_request->SearchDataLength); // overkill!
            }

        } while ('\0' != search_request->SearchData[index+1]);

        assert(index2 == file_count);

        index2 = 0;

        do {
            path_list[index2++] = &search_request->SearchData[index];

            while ('\0' != search_request->SearchData[index]) {
                index++;
                assert(index < search_request->SearchDataLength);
            }
        } while ('\0' != search_request->SearchData[index+1]);

        assert(index2 == path_count);

        // Now let's do the search and see what we get back
    ...
    niccolum_find_file_in_path(NULL, NULL, NULL, NULL, NULL);
}
